...
Deno.serve(async (request) => {
  ...
  const body = await request.json().catch(() => ({})) as { versionId?: string; documentVersionId?: string };
  // documentVersionId is accepted temporarily for backwards compatibility with
  // older platform-api deployments. New callers use the canonical versionId.
  const versionId = body.versionId ?? body.documentVersionId;
  ...
  try {
    ...
    const { error: updateError } = await supabase.from('document_versions').update({ malware_status: status, sha256: result.sha256 ?? null }).eq('id', versionId);
    if (updateError) return json({ error: 'scan_result_persist_failed' }, 500);

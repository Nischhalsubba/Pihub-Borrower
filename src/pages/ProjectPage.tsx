...
export function ProjectPage() {
  ...
  const save = (e: React.FormEvent) => {
    e.preventDefault();
    const complete = Boolean(draft.name.trim() && draft.location.trim() && draft.assetClass.trim() && draft.stage.trim() && draft.planningStatus.trim() && draft.exitStrategy.trim());
    updateSection('project', draft, complete); setMessage(complete ? 'Project information saved.' : 'Draft saved. Required project fields remain incomplete.');
  };
  return <div className="route-stage narrow-stage"><PageHead eyebrow="Borrower / Application" title="Project / Property" subtitle="Describe the asset, development stage, project economics and repayment path." />
    <form className="form-card" onSubmit={save}><div className="form-grid two"><Field label="Project name"><input required value={draft.name} onChange={(e)=>setDraft({...draft,name:e.target.value})}/></Field><Field label="Location"><input required value={draft.location} onChange={(e)=>setDraft({...draft,location:e.target.value})}/></Field></div>
      <div className="form-grid two"><Field label="Asset class"><input required value={draft.assetClass} onChange={(e)=>setDraft({...draft,assetClass:e.target.value})}/></Field><Field label="Development stage"><select value={draft.stage} onChange={(e)=>setDraft({...draft,stage:e.target.value})}><option>Planning</option><option>Acquisition</option><option>Construction</option><option>Stabilization</option><option>Operating</option></select></Field></div>

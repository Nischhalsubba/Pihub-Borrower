...
export function FinancialsPage() {
  ...
  const save = (e: React.FormEvent) => { e.preventDefault(); const complete = draft.revenue2025 > 0 && draft.ebitda2025 !== 0 && draft.equity > 0; updateSection('financials', draft, complete); setMessage(complete ? 'Financial information saved.' : 'Draft saved. Add the latest revenue, EBITDA and equity information.'); };
  return <div className="route-stage narrow-stage"><PageHead eyebrow="Borrower / Application" title="Financials" subtitle="Historical and current financial information used to assess financing capacity and structure." />
    <form className="form-card" onSubmit={save}>
      ...
      {message && <div className="success-banner" role="status">{message}</div>}<div className="form-actions"><button className="button primary" type="submit">Save financials</button></div>
    </form>
  </div>;
}
